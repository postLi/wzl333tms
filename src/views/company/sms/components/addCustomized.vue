<template>
  <el-dialog :title="popTitle" v-loading="loading" :visible.sync="isShow" :close-on-click-modal="false" :before-close="closeMe" class="tms_dialog tms_dialog_alignleft" width="500px">
    <el-form ref="formModel" :model="formModel" :rules="rules">
      <el-form-item prop="defaultSign">
        <el-input v-model="formModel.defaultSign" placeholder="请输入短信前缀" :disabled="isEdit" :maxlength="25" :minlength="1"></el-input>
      </el-form-item>
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        defaultSign: [{required: true, message: '不能为空', trigger: 'change'}]
      },
      isShow: false
    }
  },
  ...
  methods: {
    postSmsSign () {
      postSmsSign(this.otherinfo.orgid).then(data => {
        if (data) {
          this.formModel.defaultSign = data.data.modifyCount === 1 ? data.data.modifySign : data.data.defaultSign
          this.isEdit = data.data.modifyCount ===1
