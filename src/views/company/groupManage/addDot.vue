  ...
  <div class="add-dot" v-loading="loading">
    <pop-right :title='popTitle' :isShow="popVisible" @close="closeMe" class='addEmployeerPop'>
      <template class='addEmployeerPop-content' slot="content">
        <!-- 本网点是父级时显示 -->
        <el-form :model="form" :rules="rules" ref="ruleForm" class="demo-ruleForm" :inline="true" label-position="right" size="mini" :key="formKey" v-if="companyId === form.id">
          ...
          <el-form-item label="客服电话" :label-width="formLabelWidth" prop="servicePhone" clearable>
            <el-input v-model="form.servicePhone" auto-complete="off" :maxlength="30" :disabled="form.status===31"></el-input>
          </el-form-item>
        ...
        <el-form :model="form" :rules="rules" ref="ruleForm" class="demo-ruleForm" :inline="true" label-position="right" size="mini" :key="formKey" v-else>
          ...
          <el-form-item label="负责人" :label-width="formLabelWidth" prop="responsibleName">
            <el-input v-model="form.responsibleName" auto-complete="off" :disabled="form.status===31" clearable :maxlength="10"></el-input>
          </el-form-item>
          <el-form-item label="负责人电话" :label-width="formLabelWidth" prop="responsibleTelephone">
          ...
          <el-form-item label="客服电话" :label-width="formLabelWidth" prop="servicePhone" clearable>
            <el-input v-model="form.servicePhone" auto-complete="off" :disabled="form.status===31" :maxlength="30"></el-input>
          </el-form-item>
          <!--   <el-form-item label="详细地址" :label-width="formLabelWidth">
          ...
          <el-form-item label="代收款限额" :label-width="formLabelWidth" prop="">
            <el-input v-model="form.collectionFee" auto-complete="off" :maxlength="9" :disabled="form.status===31" v-number-only>
              <template slot="append">元</template>
            </el-input>
          </el-form-item>
          <el-form-item label="提现基准" :label-width="formLabelWidth" prop="">
            <el-input v-model="form.benchmark" auto-complete="off" :disabled="form.status===31" v-number-only :maxlength="9"></el-input>
          </el-form-item>
          <el-form-item label="预警额度" :label-width="formLabelWidth" prop="">
            <el-input v-model="form.warningQuota" auto-complete="off" :maxlength="9" :disabled="form.status===31" v-number-only>
              <template slot="append">元</template>
            </el-input>
          </el-form-item>
          <el-form-item label="锁机额度" :label-width="formLabelWidth" prop="">
            <el-input v-model="form.lockMachineQuota" auto-complete="off" v-number-only :maxlength="9" :disabled="form.status===31">
              <template slot="append">元</template>
            </el-input>
          </el-form-item>
          <el-form-item label="园区名称" :label-width="formLabelWidth" prop="parkName">
          ...
          <div class="ad-add-dot-admin">
            ...
            <el-form-item v-if="form.accountStatus  && !isModify" :label-width="formLabelWidth" prop="accountName">
              <el-tooltip class="item" effect="dark" placement="top" :enterable="false" :manual="true" :value="tooltip" tabindex="-1">
                <div slot="content">账号可以由字母、数字组成
                  <br/>长度范围2~15个字符</div>
                <el-input v-model.trim="form.accountName" auto-complete="off" @focus="tooltip = true" @blur="tooltip = false" style="width: 307px;"></el-input>
              </el-tooltip>
              <!--<el-input placeholder="管理员账号" v-model="form.accountName" auto-complete="off" :maxlength="20" ></el-input>-->
            </el-form-item>
          </div>
          <div class="rem-add-dot">
            <el-form-item label="详细地址" :label-width="formLabelWidth">
              <el-input type="textarea" :rows="2" v-model="form.detailedAddr" auto-complete="off" :disabled="form.status===31" :maxlength="30" placeholder="不可超30字"></el-input>
            </el-form-item>
            <el-form-item label="备注" :label-width="formLabelWidth" prop="">
              <el-input type="textarea" :rows="2" placeholder="不可超300字" v-model="form.remarks" auto-complete="off" :maxlength="300"></el-input>
            </el-form-item>
          </div>
        </el-form>
     ...
     <AddPark :isMatreg="isMatreg" :centerDialogVisible="centerDialogVisible" @close="closeAddReg" @success="getsuccess" />
  </div>
</template>
<script>
import {
...
export default {
  ...
  methods: {
    ...
    getsuccess(obj) {
      this.$set(this.form, 'parkName', obj.parkName)
      this.$set(this.form, 'parkId', obj.id)
      this.$set(this.form, 'socialCreditCode', obj.substationCode || '')
    },
    changePark(val) {
