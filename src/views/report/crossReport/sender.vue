<template>
  <div class="tab-content chartSender">
    ...
    <div class="tab_info">
     <div class="btns_box">
        <el-button type="primary" v-has:REPORT_PRINT_6 :size="btnsize" icon="el-icon-printer" @click="doAction('print')" plain>打印报表</el-button>
        <el-button type="primary" v-has:REPORT_VIEW_6 :size="btnsize" icon="el-icon-view" @click="doAction('preview')" plain>打印预览</el-button>
        <el-button type="primary" v-has:REPORT_SETTING_6 :size="btnsize" icon="el-icon-setting" @click="doAction('setting')" plain>打印设置</el-button>
      </div>
      <div class="info_tab">
...
export default {
  ...
  methods: {
    ...
    getSearchParam() {
      ...
      // }
      const path = window.location.protocol + '//' + window.location.host + '/static/supcan/crossReport.html?access_token=' + access_token + '&' + (new Date()).getTime()
