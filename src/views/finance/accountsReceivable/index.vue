<template>
  <div class="accountsReceivable-manager tab-wrapper">
    <div class="eltab clearfix">
      <el-badge :value="counts.allFeeCount" class="item" :hidden="counts.allFeeCount === 0" v-has:FINANCE_RECLIST1>
        <router-link to="/finance/accountsReceivable/all" class="tab-label">全部</router-link>
      </el-badge>
      <el-badge :value="counts.nowPayFeeCount" class="item" :hidden="counts.nowPayFeeCount === 0" v-has:FINANCE_RECLIST2>
        <router-link to="/finance/accountsReceivable/cash" class="tab-label">现付</router-link>
      </el-badge>
      <el-badge :value="counts.arrivepayFeeCount" class="item" :hidden="counts.arrivepayFeeCount === 0" v-has:FINANCE_RECLIST3>
        <router-link to="/finance/accountsReceivable/arrive" class="tab-label">到付</router-link>
      </el-badge>
      <el-badge :value="counts.receiptpayFeeCount" class="item" :hidden="counts.receiptpayFeeCount === 0" v-has:FINANCE_RECLIST4>
        <router-link to="/finance/accountsReceivable/receipt" class="tab-label">回单付</router-link>
      </el-badge>
      <el-badge :value="counts.monthpayFeeCount" class="item" :hidden="counts.monthpayFeeCount === 0" v-has:FINANCE_RECLIST5>
