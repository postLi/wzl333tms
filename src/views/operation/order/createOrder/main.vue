              ...
              <el-col :span="(eitem.fieldProperty==='shipOther' ? 8: (eitem.fieldProperty==='shipRemarks' ? 24: 4))" v-for="(eitem, eindex) in modelList" :key="eindex" v-if="eitem.templateType === 'tmsOrderShip' && eitem.hide">
                ...
                <div class="order-form-item" v-if="eitem.fieldProperty==='shipUserid'">
                  ...
                  <el-form-item :error="shipFieldValueInfo.shipUserid">
                    ...
                    <querySelect ref="tmsOrdershipUserid" show="select" filterable :orgid="otherinfo.orgid " size="mini" search="name" v-model="form.tmsOrderShip.shipUserid" @change="selectShipUserid" />
                  </el-form-item>
                </div>
...
export default {
  ...
  methods: {
    ...
    goNextEditBatch() {
      ...
      if (!this.isSaveAndNew) { // 如果保存并新增的话就按上面的逻辑执行
        // 如果是保存并新增 就不需要弹出框判断
    ...
    setPreOrder() {
      ...
      console.log('preOrderInfo:', this.form, this.form.tmsOrderShip, this.form.tmsOrderShip.shipPayWay, data)
    },
    /** 收货人/发货人 */
    setSender(item, type) {
      type = type || 'sender'
    ...
    changeFee(index, name, event) {
      ...
      if (/deliveryFee/.test(name)) {
        ...
        if (val > 0) {
          console.log('送货：：交接方式：：：', this.personConfig.shipSetKey)
          // 如果交接方式没有送货 就不生效, 且只生效一次
          if (!this.isDelivery && !this.personConfig.shipSetKey.handoverModeDict['69']) {
    ...
    reset(shouldReinputDefault) {
      ...
      if (shouldReinputDefault) {
        ...
        console.log('hhhhhhhhhhhhhhhhhhhhhhhhhhhhhh', this.form.tmsOrderShip.createTime)
            ...
            if (tmsMath.add(this.form.tmsOrderShip.shipNowpayFee, this.form.tmsOrderShip.shipArrivepayFee, this.form.tmsOrderShip.shipMonthpayFee, this.form.tmsOrderShip.shipReceiptpayFee).result() !== parseFloat(this.form.tmsOrderShip.shipTotalFee, 10)) {
              ...
              if (this.output.ismodify) {
                ...
                pro.then(res => {
                  ...
                  resolve()
                  if (this.isSavePrint) {
                    this.printLibAndShip().then(res => {}) // 执行成功后打印运单-跳转到运单列表的
                  }
                ...
                orderManage.postNewOrder(data).then(res => {
                  ...
                  if (!this.output.isbatch) {
                    ...
                    this.batchSaveList[this.currentBatch].issave = true
                    if (this.isSavePrint) {
                      this.printLibAndShip() // 执行成功后打印运单-跳转到运单列表的
                    }
