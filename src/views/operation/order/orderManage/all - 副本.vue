          ...
          <template v-for="column in tableColumn">
            <el-table-column
              ...
              <template slot="header" slot-scope="scope">
                {{ column.label }}
                <div @click.stop.prevent class="table-filter-wrapper" >
                  <input @keyup="(e)=>{filterData(e.srcElement.value, column.prop)}" type="text" class="table-filter-input">
                </div>
              </template>
            </el-table-column>
            <el-table-column
...
export default {
  ...
  methods: {
    filterData(val, prop) {
      // 每次点击上面的查询按钮或者翻页时，清空过滤框
      if (!this.filterParams) {
        this.filterParams = {}
      }
      this.filterParams[prop] = val
      this.usersArr = this.oriTableData.filter(el => {
        let flag = true
        for (const i in this.filterParams) {
          // 只有匹配了前一个才能继续匹配下一个条件
          if (flag) {
            flag = ((el[i] + '') || '').indexOf(this.filterParams[i]) !== -1
          }
        }
        return flag
      })
