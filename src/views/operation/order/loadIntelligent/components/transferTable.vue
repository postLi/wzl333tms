...
export default {
  ...
  watch: {
    ...
    resetTuckLoad: { // 参数设置新的车型列表长度
      handler(cval, oval) {
        ...
        const arr = []
        this.selectedRight = []
        const difflen = Number(cval || 0) - Number(this.orgRightTable.length ? this.orgRightTable.length : 0)
        if (difflen > 0) {
  ...
  computed: {
    ...
    showLeftStyle() {
      let showWidth = '35%'
      showWidth = this.isShowLeftTable ? '100%' : (this.isShowRightTable ? '0%' : '35%')
      return {
        width: showWidth,
        transition: '0.3s'
      }
    ...
    showRightStyle() {
      let showWidth = '65%'
      showWidth = this.isShowRightTable ? '100%' : (this.isShowLeftTable ? '0%' : '65%')
      return {
        width: showWidth,
        transition: '0.3s'
      }
  ...
  methods: {
    ...
    goLeft() { // 右边穿梭到左边
      this.selectedRight.forEach((e, index) => {
        const find = this.leftTable.filter(em => {
          return em.repertoryId === e.repertoryId
        })
        if (find.length === 0) {
    ...
    goRight() { // 左边穿梭到右边
      if (this.dofoLen === 0) {
        ...
        this.selectedLeft.forEach((e, index) => {
          const find = this.rightTable.filter(em => {
            return em.repertoryId === e.repertoryId
          })
          if (find.length === 0) {
