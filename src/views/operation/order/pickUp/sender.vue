<template>
  <div class="tab-content miniHeaderSearch" v-loading="loading">
    ...
    ></TableSetup>
    <PickupMain
      :popVisible.sync="pickMaintainisible"
      :isDepMain="isDepMain"
      @close="openpickMaintainisible"
      @success="fetchData"
      :dotInfo="selectInfo"
    ></PickupMain>
    <PickupRelevance
      :popVisible.sync="releMaintainisible"
      :isDepMain="isDepMain"
      @close="openpickReletainisible"
      @success="fetchData"
      :dotInfo="selectInfo"
    ></PickupRelevance>
  </div>
...
export default {
  ...
  methods: {
    ...
    fetchAllCustomer() {
      this.loading = true
      return fetchPostlist(this.searchQuery)
        .then(data => {
