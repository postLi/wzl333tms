    ...
    doAction (type) {
      ...
      switch (type) {
          ...
          case 'modify':
              ...
              this.$router.push({path: '/operation/order/transferLoad/' + this.selectInfo.transferBatchNo })
              break;
          // 跟踪中转
          case 'track':
                  if(this.selected.length > 1){
          ...
          // 取消中转
          case 'cancel':
                let avaiableItem = this.selected.filter(el => {
                ...
                if(!avaiableItem.length){
                  ...
                  // 获取批次
                  let transferBatchNos = avaiableItem.map(el => {
                    return el.transferBatchNo
                  })
                  // 获取运单号
                  let shipSns = avaiableItem.map(el => {
                    return el.id
                  })
