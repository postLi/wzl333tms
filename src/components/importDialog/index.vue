<template>
  <!-- 批量导入Excel弹出框 -->
  <el-dialog :title="titlePop" :visible.sync="isShow" :close-on-click-modal="false" :before-close="closeMe" class="tms_dialog tms_dialog_import" width="500px">
    ...
    <div class="import_result" v-else>
      <el-progress :text-inside="true" :stroke-width="15" :percentage="percentage" :status="(uploadStatus&& percentages>40)?'success':'exception'" v-if="isProgress"></el-progress>
      <p v-else>{{resMessage.resultInfo}}</p>
      <el-table ref="multipleTable" :data="resMessage.failInfoList" stripe border tooltip-effect="dark" :default-sort="{prop: 'id', order: 'ascending'}" style="width: 100%;margin-top: 10px;" :key="tableKey" :class="(resMessage.failInfoList &&resMessage.failInfoList.length)>11?'failinScroll':''">
