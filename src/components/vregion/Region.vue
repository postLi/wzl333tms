      ...
      watch: {
        ...
        nowProvince(value) {
          if (this.pauseWatch) return
            // 允许城市级别
          if (this.city) {
            this.fetchCity(value, 'city').then(res => {
                        // 如果不是当前选取的项，则不进行保存渲染
      ...
      methods: {
        ...
        showDropdown(val) {
          ...
          if (typeof (val) === 'boolean') {
            ...
            let ifZhixia = false
            const zhixiashi = ['北京', '天津', '重庆', '上海', '香港', '澳门', '台湾']
        ...
        fetchCity(code = 0, type) {
          return new Promise((resolve) => {
            if (type === 'city') {
              // 如果是获取市，则判断是否为直辖市/特别行政区
              let name
              let oldres
              if (code === 110000) {
                name = '北京'
            ...
            return getCityByCode(code).then(res => {
              res.code = res.data.pid
              res.data = res.data.regionQueryList
